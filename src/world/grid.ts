...
import { GridActor } from './world-tiles'

export type Grid = (null | GridActor)[][]
export type IntGrid = number[][]

/**
 * Methods for making and handling grids.
 */
export const makeGrid = (width:number, height:number):Grid => {
  const grid = []
...
export const makeIntGrid = (grid:Grid, collisions:number[][]):IntGrid => {
  const newGrid = []
